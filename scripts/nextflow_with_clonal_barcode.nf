...
process extract_barcode {
    label "small"
    conda osn_conda_env
    publishDir path: "${params.out}/${id}", mode: "copy", pattern: "*.bam"

    input:
        tuple val(id), path(cell)

    output:
        tuple val(id), path('possorted_barcode_with_header.bam')

    script:
    """
    echo "id: $id, cell: $cell"
    which extract_barcode
    extract_barcode $cell/possorted_genome_bam.bam
    """
}
...
workflow {
    ...
    extract_barcode(cellranger_count.out.out_folder)
    extract_barcode.out.view()
